...
from backend.configs.constants import DOCUMENT_PARSER_LOGGING, FILENAME_LENGTH_MAX
...
def search_for_url(line: str) -> tuple[str | None, str | None]:
...
def save_pdf_by_url(
    response: requests.Response, name: str | None, url: str, output_dir: Path
    ...
    filename = (
        filename[-FILENAME_LENGTH_MAX:]
        if len(filename) > FILENAME_LENGTH_MAX
        else filename
    )
...
def save_image_by_url(
    ...
    if not clean_name:
        ...
        clean_name = (
            clean_name[-FILENAME_LENGTH_MAX:]
            if len(clean_name) > FILENAME_LENGTH_MAX
            else clean_name
        )
    try:
...
def save_text_by_url(
    name: str | None, url: str, output_dir: Path
) -> tuple[TextParsingStatus, Path]:
