...
from backend.configs.constants import (
    DOCUMENT_PARSER_LOGGING,
    EXCLUDED_EMBED_METADATA_KEYS,
    EXCLUDED_LLM_METADATA_KEYS,
    HAS_PARSED_URL_MARKER,
    HAS_UNPARSED_URL_MARKER,
    REMNOTE_IMAGE_HOST_MARKER,
)
from backend.configs.enums import (
    ContentType,
    ExternalSourceUrlType,
    ImageParsingStatus,
    PDFParsingStatus,
    StorageType,
)
...
class RemNoteParser:
    ...
    def __init__(self, path_settings: PathSettings, storage_settings: StorageSettings):
        ...
        self.kg_storage = KnowledgeGraphStorage(
            path_settings,
            storage_settings,
            local_storage=path_settings.local_storage_dir,
        )
    ...
    def add_text_nodes(self, text_nodes: list[TextNode], allow_update: bool = True):
        ...
        self.kg_storage.storage_context.docstore.add_documents(
    ...
    def create_text_node(
        ...
        text: str,
        node_stack: list[TextNode | None],
    ) -> tuple[TextNode, list[TextNode | None]]:
        ...
        if depth_level > 0 and node_stack:
            ...
            if parent_node:
                ...
                new_node.relationships[NodeRelationship.PARENT] = RelatedNodeInfo(
                    node_id=parent_node.node_id, metadata={"title": parent_node.text}
                )
                ...
                parent_node.relationships[NodeRelationship.CHILD].append(
                    RelatedNodeInfo(
                        node_id=new_node.node_id, metadata={"title": parts[-1]}
                    )
                )
    ...
    def parse_markdown_file(
        self, file_path: Path, check_url: bool = False
    ) -> list[TextNode]:
        ...
        try:
            with open(file_path, encoding="utf-8") as f:
                for line_number, line in enumerate(f):
                    ...
                    if (
                        not cleaned_content.strip()
                        and REMNOTE_IMAGE_HOST_MARKER in raw_content
                    ):
                    ...
                    full_context_text = f"{filename} > " + " > ".join(
                        [c for c in context_stack if c != "..."]
                    )
    ...
    def update_text_nodes(
        ...
        childs = []
        for node in nodes:
            node.metadata["depth_level"] = (
                node.metadata["depth_level"]
                + parent_node_info.metadata["depth_level"]
                + 1
            )
            ...
            node.metadata["path"] = (
                parent_node_info.metadata["path"] + node.metadata["path"]
            )
        ...
        childs_related_info = [
            RelatedNodeInfo(node_id=node.node_id, metadata={"title": node.text})
            for node in childs
        ]
        parent_node_info.relationships[NodeRelationship.CHILD].extend(
            childs_related_info
        )
    ...
    def parse_external_links(self):
        ...
        for node_id, node_info in tqdm(
            nodes_iter, desc="Updating nodes with external urls"
        ):
            ...
            try:
                ...
                if ContentType.application_json.value in content_type:
                    status, saved_file_path = save_pdf_by_url(
                        response, name, url, self.path_settings.parsed_pdfs_dir
                    )
                    ...
                    status, saved_file_path = save_image_by_url(
                        ...
                        name,
                        url,
                        self.path_settings.parsed_images_dir,
                    )
                    if status in (
                    ...
                    x.value in content_type
                    for x in ContentType
                    if x.name.startswith("text")
                ):
                    _, mkd_file_path = save_text_by_url(
                        name, url, self.path_settings.parsed_texts_dir
                    )

                if mkd_file_path:
                    nodes = self.parse_markdown_file(mkd_file_path, check_url=False)
                    if nodes:
                        nodes, node_info, nodes_to_update = self.update_text_nodes(
                            nodes, node_info, nodes_to_update
                        )

            except Exception:
                logger.error(
                    f"Exception while parsing external url ({url})", exc_info=True
                )
                ...
                if node_info.metadata.get(HAS_UNPARSED_URL_MARKER):
                    ...
                    if not changed:
                        if node_info.parent_node:
                            ...
                            child_nodes_updated = [
                                n
                                for n in parent_node.child_nodes
                        ...
                        self.kg_storage.storage_context.docstore.delete_document(
                            node_id
                        )
                        deleted_nodes_num += 1
    ...
    def run(self):
        ...
        assert self.kg_storage.storage_context.docstore.docs, (
            "No docs found in the docstore"
        )
