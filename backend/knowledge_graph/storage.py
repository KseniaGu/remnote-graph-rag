...
from backend.knowledge_graph.custom_types import (
    CustomMemgraphPropertyGraphStore,
    CustomNeo4jPropertyGraphStore,
)
...
class KnowledgeGraphStorage:
    ...
    def __init__(
        self, path_settings: PathSettings, storage_settings: StorageSettings, **kwargs
    ) -> None:
    ...
    def get_document_storage(self, **kwargs) -> Any:
        ...
        if self.storage_settings.document_storage.storage_type == StorageType.local:
            ...
            if self.not_empty(storage_path):
                document_storage = SimpleDocumentStore.from_persist_dir(
            ...
            if self.storage_settings.document_storage.password:
                redis_url = self.storage_settings.document_storage.get_connection_url(
                    driver="rediss"
                )
                redis_client = redis.from_url(redis_url)
                kv_store = RedisKVStore(
                    redis_client=redis_client, namespace="llama_index"
                )
            ...
            if storage_is_empty:
                local_path_to_check = kwargs.get(
                ...
                if self.not_empty(local_path_to_check):
                    ...
                    document_storage.add_documents(
                        list(local_document_storage.docs.values())
                    )
        else:
            raise ValueError(
                "Only Local and Redis storage types are supported for Document Storage"
            )
    ...
    def get_index_storage(self, **kwargs) -> Any:
        ...
        if self.storage_settings.index_storage.storage_type == StorageType.local:
            ...
            if (
                self.not_empty(storage_path)
                and (storage_path / "index_store.json").exists()
            ):
                index_storage = SimpleIndexStore.from_persist_dir(
                    persist_dir=str(storage_path)
                )
            else:
            ...
            if self.storage_settings.index_storage.password:
                redis_url = self.storage_settings.index_storage.get_connection_url(
                    driver="rediss"
                )
                redis_client = redis.from_url(redis_url)
                kv_store = RedisKVStore(
            ...
            if storage_is_empty:
                ...
                local_index_storage = SimpleIndexStore.from_persist_dir(
                    persist_dir=str(local_path_to_check)
                )
                index_storage.add_index_struct(local_index_storage.get_index_struct())
        else:
            raise ValueError(
                "Only Local and Redis storage types are supported for Index Storage"
            )
    ...
    def get_vector_storage(self, **kwargs) -> Any:
        ...
        if self.storage_settings.vector_storage.storage_type == StorageType.local:
            ...
            if (
                self.not_empty(storage_path)
                and (storage_path / "default__vector_store.json").exists()
            ):
                vector_storage = SimpleVectorStore.from_persist_dir(
                    persist_dir=str(storage_path)
                )
            else:
            ...
            if self.storage_settings.vector_storage.password:
                redis_connection = (
                    self.storage_settings.vector_storage.get_connection_url(
                        driver="rediss"
                    )
                )
            else:
            ...
            vector_storage = RedisVectorStore(
                ...
                overwrite=self.storage_settings.vector_storage.overwrite_index,
                schema=schema,
            )
        elif self.storage_settings.vector_storage.storage_type == StorageType.pinecone:
            pc = Pinecone(
                api_key=self.storage_settings.vector_storage.api_key.get_secret_value()
            )
            ...
            raise ValueError(
                "Only Local, Redis, and Pinecone storage types are supported for Vector Storage"
            )
    ...
    def get_property_graph_storage(self, **kwargs) -> Any:
        ...
        """
        if (
            self.storage_settings.property_graph_storage.storage_type
            == StorageType.local
        ):
            storage_path = self.storage_settings.property_graph_storage.storage_path
            if (
                self.not_empty(storage_path)
                and (storage_path / "property_graph_store.json").exists()
            ):
                property_graph_storage = SimplePropertyGraphStore.from_persist_dir(
                    persist_dir=str(storage_path)
                )
            else:
            ...
            self._init_remote_property_graph_from_local(
                property_graph_storage, **kwargs
            )
        elif (
            self.storage_settings.property_graph_storage.storage_type
            == StorageType.memgraph
        ):
            ...
            raise ValueError(
                "Only Local, Neo4j, and Memgraph storage types are supported for Property Graph Storage"
            )

        return property_graph_storage

    def _init_remote_property_graph_from_local(
        self, property_graph_storage: Any, **kwargs
    ) -> None:
        ...
        if (
            self.storage_settings.property_graph_storage.init_from_local
            and not not_empty
        ):
            local_path_to_check = kwargs.get(
                "local_storage", self.path_settings.local_storage_dir
            ...
            if self.not_empty(local_path_to_check):
                local_property_graph_storage = (
                    SimplePropertyGraphStore.from_persist_dir(
                        persist_dir=str(local_path_to_check)
                    )
                )
                ...
                property_graph_storage.upsert_relations(
                    list(local_property_graph_storage.graph.relations.values())
                )
    ...
    def _init_storage_context(self, **kwargs):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            property_graph_store_future = executor.submit(
                self.get_property_graph_storage, **kwargs
            )
