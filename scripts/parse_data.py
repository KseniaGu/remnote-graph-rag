...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Parse RemNote markdown data into local LlamaIndex document storage."
    )
    parser.add_argument(
        "--output-root",
        default=None,
    ...
    parser.add_argument(
        "--raw-data-dir", default=None, help="Raw RemNote markdown directory to parse."
    )
    parser.add_argument(
        "--parsed-pdfs-dir", default=None, help="Directory for downloaded/parsed PDFs."
    )
    parser.add_argument(
        "--parsed-images-dir",
        default=None,
        help="Directory for downloaded/parsed images.",
    )
    parser.add_argument(
        "--parsed-texts-dir",
        default=None,
        help="Directory for downloaded/parsed HTML/text files.",
    )
    parser.add_argument(
        "--local-storage-dir",
        default=None,
        help="Directory for local LlamaIndex storage files.",
    )
...
def make_path_settings(args: argparse.Namespace) -> PathSettings:
    ...
    return PathSettings(
        ...
        if args.raw_data_dir
            ...
            output_root / "raw" / "AI Research"
            if output_root
            else defaults.raw_data_dir
        ),
        ...
        if args.parsed_images_dir
            ...
            output_root / "parsed_images" if output_root else defaults.parsed_images_dir
        ),
        parsed_texts_dir=Path(args.parsed_texts_dir)
        if args.parsed_texts_dir
        else (
            output_root / "parsed_texts" if output_root else defaults.parsed_texts_dir
        ),
