...
def test_analyst_and_mentor_output_caps_match_for_vllm(monkeypatch) -> None:
    ...
    assert settings.analyst.max_tokens == 4096
...
def test_active_prompt_metadata_uses_the_runtime_output_cap() -> None:
    ...
    active_prompts = (
        prompt_root / "analyst" / "v6.yaml",
        prompt_root / "analyst" / "v4.yaml",
        prompt_root / "mentor" / "v4.yaml",
    )

    for prompt_path in active_prompts:
        assert "num_predict: 4096" in prompt_path.read_text(encoding="utf-8")
