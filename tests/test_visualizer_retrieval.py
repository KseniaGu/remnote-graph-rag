...
class FakeGraphStore:
    def __init__(self, nodes: list[FakeGraphNode], triplets: list[tuple] | None = None) -> None:
...
def make_indexer(vector_store, graph_store, docs=None, settings=None):
    storage_context = SimpleNamespace(docstore=SimpleNamespace(docs=docs or {}), vector_store=vector_store)
...
class VisualizerRetrievalPipelineTests(unittest.TestCase):
    def test_exact_anchor_resolution_emits_semantic_concept_relation(self) -> None:
        text_classification = FakeGraphNode("concept_text", "Text Classification", aliases=["text classification"])
        classifier = FakeGraphNode("concept_classifier", "Classifier")
        relation = FakeRelation("USES", {"evidence_chunk_ids": ["chunk_1"], "max_confidence": 0.9})
    ...
    def test_mentions_discover_concepts_but_do_not_render_mentions_or_chunks(self) -> None:
        ...
        graph_store = FakeGraphStore(
            ...
            [
                (chunk, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_1"]}), text_classification),
    ...
    def test_comparison_query_keeps_both_anchors_and_prefers_compares_to(self) -> None:
        ...
        graph_store = FakeGraphStore(
            ...
            [
                (
                    logistic,
                    FakeRelation("COMPARES_TO", {"evidence_chunk_ids": ["chunk_compare"], "max_confidence": 0.9}),
                    naive_bayes,
                )
            ],
        )
    ...
    def test_shape_graph_drops_isolated_non_anchor_candidates_by_default(self) -> None:
        ...
        text_classification = FakeGraphNode("concept_text", "Text Classification", source_chunk_ids=["chunk_1"])
        classifier = FakeGraphNode("concept_classifier", "Classifier", source_chunk_ids=["chunk_1"])
        word_dropout = FakeGraphNode("concept_dropout", "Word Dropout", source_chunk_ids=["chunk_1"])
        chunk = FakeGraphNode("chunk_1", "Chunk", node_label="text_chunk")
        graph_store = FakeGraphStore(
            [text_classification, classifier, word_dropout, chunk],
            [
                (text_classification, FakeRelation("USES", {"evidence_chunk_ids": ["chunk_1"]}), classifier),
                (chunk, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_1"]}), word_dropout),
            ],
        )
        docs = {
            "chunk_1": FakeTextNode(
        ...
        vector_store = FakeVectorStore({"postprocessed_retrieval_chunk": ["chunk_1"]})
        pipeline = VisualizerRetrievalPipeline(make_indexer(vector_store, graph_store, docs, settings=settings))
    ...
    def test_anchor_source_filter_prevents_cross_document_mentions(self) -> None:
        text_classification = FakeGraphNode("concept_text", "Text Classification", source_chunk_ids=["chunk_text"])
        classifier = FakeGraphNode("concept_classifier", "Classifier", source_chunk_ids=["chunk_text"])
        ...
        graph_store = FakeGraphStore(
            ...
            [
                (chunk_text, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_text"]}), classifier),
                (chunk_clip, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_clip"]}), clip),
            ],
        )
        docs = {
        ...
        vector_store = FakeVectorStore({"postprocessed_retrieval_chunk": ["chunk_clip", "chunk_text"]})
        pipeline = VisualizerRetrievalPipeline(make_indexer(vector_store, graph_store, docs))
    ...
    def test_multi_query_focus_does_not_add_synthetic_edges_to_mentioned_noise(self) -> None:
        naive_bayes = FakeGraphNode("concept_nb", "Naive Bayes", source_chunk_ids=["chunk_compare"])
        logistic = FakeGraphNode("concept_lr", "Logistic Regression", source_chunk_ids=["chunk_compare"])
        ag_news = FakeGraphNode("concept_ag", "AG News", source_chunk_ids=["chunk_compare"])
        chunk = FakeGraphNode("chunk_compare", "Comparison Chunk", node_label="text_chunk")
        graph_store = FakeGraphStore(
            ...
            [
                ...
                (chunk, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_compare"]}), ag_news),
            ],
        )
        docs = {
        ...
        vector_store = FakeVectorStore({"postprocessed_retrieval_chunk": ["chunk_compare"]})
        pipeline = VisualizerRetrievalPipeline(make_indexer(vector_store, graph_store, docs))
    ...
    def test_generic_related_to_is_dropped_when_specific_edge_exists_for_same_pair(self) -> None:
        clip = FakeGraphNode("concept_clip", "CLIP")
        supervision = FakeGraphNode("concept_supervision", "Natural Language Supervision")
        graph_store = FakeGraphStore(
            [clip, supervision],
            [
                (clip, FakeRelation("RELATED_TO", {"evidence_chunk_ids": ["chunk_1"]}), supervision),
                (clip, FakeRelation("USES", {"evidence_chunk_ids": ["chunk_1"]}), supervision),
            ],
        )
        pipeline = VisualizerRetrievalPipeline(make_indexer(FakeVectorStore(), graph_store))
        ...
        self.assertNotIn(("concept_clip", "RELATED_TO", "concept_supervision"), triplets)

    def test_visualization_usefulness_and_generality_affect_edge_selection(self) -> None:
        settings = KnowledgeGraphSearchSettings(
            analyst_reranker_mode="disabled",
        ...
        pipeline = VisualizerRetrievalPipeline(make_indexer(FakeVectorStore(), graph_store, settings=settings))
    ...
    def test_short_aliases_do_not_create_unrelated_exact_anchors(self) -> None:
        text_classification = FakeGraphNode("concept_text", "Text Classification", source_chunk_ids=["chunk_text"])
        loss_function = FakeGraphNode("concept_loss", "Loss Function", aliases=["L"], source_chunk_ids=["chunk_loss"])
        hidden_state = FakeGraphNode("concept_h", "$\\mathbf{h}$", aliases=["h"], source_chunk_ids=["chunk_h"])
        classifier = FakeGraphNode("concept_classifier", "Classifier", source_chunk_ids=["chunk_text"])
        graph_store = FakeGraphStore(
    ...
    def test_short_alias_substring_does_not_block_vector_anchor_fallback(self) -> None:
        ...
        input_x = FakeGraphNode("concept_input_x", "Input x", aliases=["x"], source_chunk_ids=["chunk_input"])
        ...
        vector_store = FakeVectorStore({"postprocessed_concept_node": ["concept_fasttext"]})
        pipeline = VisualizerRetrievalPipeline(make_indexer(vector_store, graph_store))
    ...
    def test_semantic_expansion_uses_grounded_mentioned_concepts(self) -> None:
        ...
        graph_store = FakeGraphStore(
            ...
            [
                (chunk, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_text"]}), multi_label),
                (
                    multi_label,
                    FakeRelation("USES", {"evidence_chunk_ids": ["chunk_text"]}),
                    bce,
        ...
        vector_store = FakeVectorStore({"postprocessed_retrieval_chunk": ["chunk_text"]})
        pipeline = VisualizerRetrievalPipeline(make_indexer(vector_store, graph_store, docs, settings=settings))
    ...
    def test_grounded_typed_edges_beat_synthetic_fallback_edges(self) -> None:
        ...
        useful = FakeGraphNode("concept_useful", "Useful Method", source_chunk_ids=["chunk_1"])
        detail = FakeGraphNode("concept_detail", "Useful Detail", source_chunk_ids=["chunk_1"])
        generic = FakeGraphNode("concept_generic", "Generic Concept", source_chunk_ids=["chunk_1"])
        chunk = FakeGraphNode("chunk_1", "Chunk", node_label="text_chunk")
        graph_store = FakeGraphStore(
            [anchor, useful, detail, generic, chunk],
            [
                (chunk, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_1"]}), useful),
                (chunk, FakeRelation("MENTIONS", {"evidence_chunk_ids": ["chunk_1"]}), generic),
                (
                    useful,
    ...
    def test_node_and_edge_budgets_are_enforced(self) -> None:
        ...
        neighbors = [FakeGraphNode(f"concept_{idx}", f"Neighbor {idx}") for idx in range(6)]
        graph_store = FakeGraphStore(
            [anchor, *neighbors],
            [
                (anchor, FakeRelation("USES", {"evidence_chunk_ids": [f"chunk_{idx}"]}), neighbor)
